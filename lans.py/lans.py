...
def LANsMain(args):
    ...
    '''shell
    xy@kali:~$ /sbin/ip route （显示路由表）

    default via 172.28.32.1 dev wlan1  proto static  metric 1024
    172.28.32.0/20 dev wlan1  proto kernel  scope link  src 172.28.41.63
    202.112.144.235 via 172.28.32.1 dev wlan1  proto dhcp  metric 10
    '''
    ...
    if args.routerip:
        ...
        try:
            # 从上面的输出拿到路由IP
            routerIP = ipr[2]  # 得到172.28.32.1
        except:
            exit("You must be connected to the internet to use this.")
    for r in iprs:
        if '/' in r:
            IPprefix = r.split()[0]  # 得到网段172.28.32.0/20，用于之后扫描
    ...
    if args.ipaddress:
        ...
        # 读取网段内用户IP和MAC信息，并在此基础上开mon模式进行抓包，
        # 然后处理输出格式展现给用户，让用户选择攻击对象。
    ...
    def http_parser(self, load, ack, dport):
        ...
        if url:
            ...
            if self.args.urlspy:
                ...
                domainFilterList = ['adzerk.net', 'adwords.google.com', 'googleads.g.doubleclick.net',
...
class active_users():
    ...
    current_time = 0
    monmode = ''
    # 处理数据包并形成输出格式，与用户交互 （[*] Hit Ctrl-C at any time to stop and choose a victim IP）
    def pkt_cb(self, pkt):
    ...
    # 根据传入的网段和路由器IP，使用NMAP进行ARP扫描,获取次此网段设备的IP和MAC
    def users(self, IPprefix, routerIP):
        ...
        try:
            ...
            '''
            示例：此时nmap=[
                'Nmap scan report for 172.28.32.1',
                'Host is up (0.017s latency).',
                'MAC Address: 00:0F:E2:67:6E:AB (Hangzhou H3C Technologies Co.)',
                'Nmap scan report for 172.28.32.4',
                'Host is up (0.0071s latency).',
                'MAC Address: 3C:E5:A6:58:B9:AA (Hangzhou H3C Technologies Co.)',
                'Nmap scan report for 172.28.41.63',
                'Host is up.'
            ]
            '''
        except Exception:
        ...
        '''
        在示例中，IPandMAC=[
            ['172.28.32.1', '00:0F:E2:67:6E:AB'],
            ['172.28.32.4', '3C:E5:A6:58:B9:AA']
        ]
        ...
        try:
            ...
            '''示例执行结果：
            Doing NBT name scan for addresses from 172.28.32.0/24

            IP address       NetBIOS Name     Server    User             MAC address
            ------------------------------------------------------------------------------
            172.28.32.0	Sendto failed: Permission denied
...
def print_vars(DHCPsrvr, dnsIP, local_domain, routerIP, victimIP):
    ...
    logger.write("[*] Route 	r IP: " + routerIP + '\n')
